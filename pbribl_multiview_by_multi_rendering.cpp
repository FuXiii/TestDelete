...
public:
	...
	void buildCommandBuffers()
	{
		...
		for (size_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			auto is_row_and_column_valid = [&]()->bool
			{
				if (row > 0 && column > 0)
				{
					return true;
			...
			if (is_row_and_column_valid())
			{
				for (size_t row_index = 0; row_index < row; row_index++)
				{
					for (size_t column_index = 0; column_index < column; column_index++)
					{
						...
						{
							sub_viewport.x = sub_viewport_x;
							sub_viewport.y = sub_viewport_y;
							sub_viewport.width = sub_viewport_width;
							sub_viewport.height = sub_viewport_height;
							sub_viewport.minDepth = 0.0;
							sub_viewport.maxDepth = 1.0;
						}

						//VkViewport viewport = vks::initializers::viewport((float)width * 0.5, (float)height * 0.5, 0.0f, 1.0f);
						VkViewport viewport = sub_viewport;
